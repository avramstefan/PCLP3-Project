...
#define XOR(a, b) (dll_node*)((intptr_t)(a)^(intptr_t)(b))
...
// dll_node* XOR(dll_node *x, dll_node *y) {
//     return (dll_node*)((uintptr_t)(x) ^ (uintptr_t)(y));
// }
...
*/ 
void create_deck(dll_node **deck, register int n) {
	*deck = ALLOC(dll_node, 1);
	DIE(NULL == deck, "Malloc failed");
	(*deck)->next = NULL;
	(*deck)->prev = NULL;

	dll_list *temp = ALLOC(dll_list, 1);
...
void create_card(dll_node **card, int val, char symb[MAX_SYMBOL_SIZE]) {
	*card= ALLOC(dll_node, 1);
	DIE(NULL == card, "Malloc failed");
	(*card)->next = NULL;
	(*card)->prev = NULL;

	dll_card *temp = ALLOC(dll_card, 1);
	DIE(NULL == temp, "Malloc failed");
	temp->value = val;
...
static inline int check_if_valid(int val, char symb[MAX_SYMBOL_SIZE]) {
	if (val < 1 || val > 14)
		return 0;
...
static inline int check_if_number(char val[MAX_SYMBOL_SIZE]) {
...
static inline int check_if_valid_symb_input(char symb[MAX_SYMBOL_SIZE]) {
	if (strstr(symb, " "))
		return 0;
	return 1;
}
...
*/
void add_cards_in_deck(dll_node *deck, register int n) {
	int index = 0;
	while (index < n) {
...
*/
void add_deck(dll_list *list, register int n) {
	dll_node *deck;
	create_deck(&deck, n);
...
void show_deck(dll_list *list, register int index) {
	...
	for (register int i = 0; i < deck_list->size; i++) {
...
void show_all(dll_list *list) {
	...
	for (register int i = 0; i < list->size; i += 5) {
		show_deck(list, i);
		show_deck(list, i + 1);
		show_deck(list, i + 2);
		show_deck(list, i + 3);
...
void print_deck_len(dll_list *list, register int index) {
	dll_node *deck_node = list->head;
	register int index_2 = index;
...
*/
void reverse_deck(dll_list **list, register int index) {
	dll_node *deck_node = (*list)->head;
...
*/
void del_deck(dll_list **list, register int index, register int option) {
	register int cp_index = index;
...
*/
void del_card(dll_list **list, register int deck_index, register int card_index) {
	dll_node *deck_node = (*list)->head;
	register int cp_deck_index = deck_index;
...
void add_cards_function(dll_list *list, register int index, register int nr_cards) {
	dll_node *deck_node = list->head;
	register int cp_index = index;
...
*/
void shuffle_deck(dll_list *list, register int index) {
	dll_node *deck_node = list->head;
	register int cp_index = index;
...
*/
static inline void interclass_cards(register int new_size, dll_node **card, dll_node **card_1,
						dll_node **card_2, register int size_1, register int size_2) {
...
*/
void merge_decks(dll_list **list, register int index1, register int index2) {
	if (index2 < 0 || index2 >= (*list)->size) {
		printf("The provided index is out of bounds for the deck list.\n");
		return;
	}
	dll_node *deck_node_1 = (*list)->head, *deck_node_2 = (*list)->head;
	register int cp_index1 = index1, cp_index2 = index2;
	...
	dll_node *new_deck;
	register int size_1 = deck_list_1->size;
	register int size_2 = deck_list_2->size;
	register int new_size = size_1 + size_2;
	...
	if (cp_index1 == cp_index2) {
		...
		for (register int i = 0; i < new_deck_list->size - 1; i++)
			card = card->next;
		card->next = NULL;
	} else {
		if (cp_index1 < cp_index2) {
...
* initial deck.
*/
void split_deck(dll_list **list, register int index, register int split_index) {
	register int cp_index = index;
	...
	for (register int i = 0; i < split_index; i++) {
		previous = card;
		card = card->next;
	}
...
static inline void swap_cards(dll_node **card1, dll_node **card2) {
...
static inline void check_and_swap(dll_node *card, register int i, register int j) {
...
void sort_deck(dll_list *list, register int index) {
	register int cp_index = index;
	...
	for (register int i = 0; i < deck_list->size - 1; i++) {
		for (register int j = i + 1; j < deck_list->size; j++) {
...
int validate_command(dll_list *list, char com[MAX_COMMAND_SIZE],
				register int *param1, register int *param2, int option) {
